...
### Session - FB story downloader permalink + token parse fix (2026-08-19)
- **User bug report (Hinglish):** story downloader error - "This story could not be downloaded. Facebook stories expire after 24 hours or may be private" (story NO_MEDIA 500).
- **ROOT CAUSE 1 - token parse gap** (src/lib/facebook-url.ts): STORY_PATH_RE token class was [A-Za-z0-9_=-]{4,64} - WhatsApp/share-button story links carry the PERCENT-ENCODED token (".../UzpfSVND...MjU%3D" instead of "...MjU="), so %3D tokens failed the regex and the link fell through to generic invalid. FIX: token class now [A-Za-z0-9_=%\-]{4,96}; decodeStoryToken runs decodeURIComponent(token) FIRST (then base64url -> "S:_ISC:{story_id}" -> numeric story id). Verified: raw token AND %3D-encoded token both decode to 1760346128498925.
- **ROOT CAUSE 2 - wrong story permalink:** token stories sanitized to video.php?v={storyId} - but FB stories are served via the classic story.php?story_fbid={storyId}&id={userId} permalink (the page carrying playable_url/playable_url_quality_hd JSON that extractMediaFromPageHtml reads - the documented story technique). FIX: token stories -> permalink with BOTH ids. story.php/permalink.php + story_fbid param now parse as linkType:'story' with story_fbid as the media id (was video_page with id precedence bug - "id" (USER id) won over story_fbid -> video.php?v={userId} -> certain NO_MEDIA).
- **Verified:** 18-case parse test ALL PASS (raw/encoded/no-token /stories/..., stories.php?profile_id, story.php + permalink.php permalinks -> all linkType:'story' correct media id; video.php/reel/watch/profile-video/share-r/share-v/share-p/photo/garbage regressions PASS). fetchFacebookStory still runs permalink + video.php?v= variants in parallel (existing). Live: story POST on dev IP -> 500 story-copy (KNOWN dev-IP shell - all FB pages 400-shell here; permalink is the production-IP path); story mode + non-story link -> 422 correct; video share/r regression -> 200 full media (HD 720p + SD + cover, dur 16.866); photo share/p -> 200 (5 photos cached). astro check 0/0/0, build green.
- NOTE: no REAL fresh story link available to smoke the success path - production IPs serve the story.php permalink page with video JSON (documented); re-verify with a fresh story link on a clean IP (kill node + npm run dev restart pattern).
- UNCOMMITTED (same worktree): FB video pipeline parallelization (10.6s->8.2s), premium HD tile styling (gradient border + sheen + HD badge chip in FacebookDownloadForm.astro), AGENTS.md WIP, this log.
